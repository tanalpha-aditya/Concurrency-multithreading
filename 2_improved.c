    ...
    while (1)
    {
        ...
        if (scanf("%d %d %d", &arr_customer[customer_num].cus_number, &arr_customer[customer_num].time_arrival, &arr_customer[customer_num].order_number) == 3)
        {
            ...
            for (int i = 0; i < arr_customer[customer_num].order_number; i++)
            {
                ...
                int k = 0;
                char toppingName[100]; // Temporary buffer to hold topping name
                int scanResult;
                ...
                while (true)
                {
                    ...
                    if (c == '\n' || c == EOF)
                    {                     // If it's a newline or EOF, break out of the loop
                        ungetc(c, stdin); // Put the character back for the next read
                        break;
                    }

                    ungetc(c, stdin); // Put the character back to read the full topping name

                    int scanResult = scanf("%99s", toppingName); // Read a topping name, up to 99 characters
                    if (scanResult == 1)
                    { // Successfully read a topping name
                        strcpy(arr_customer[customer_num].order[i].topping_name[k], toppingName);
                        k++;
                    }
                    else
                    {
                        break; // Break on unsuccessful read
                    }
                }
...
// cookies 2
// 1 0 2
// vanilla almonds sprinkles cookies
// chocolate cookies sprinkles
// 2 0 1
// blueberry sprinkles
// 3 2 2
// chocolate cookies syrup
// blueberry
// 4 3 2
// vanilla syrup almonds
// chocolate cookies almonds
// 5 7 1
// blueberry almonds
// 6 7 1
// vanilla almonds
